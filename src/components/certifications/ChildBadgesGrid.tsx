 ...
 *  the same size on all five cards and identical to ChildBadgesGrid's. */
 ...
 *
 *  EVERY UNIT HERE HAS A BADGE. There used to be a second arm rendering a
 *  badge-less unit as a dashed numbered tile; its only occupants were the
 *  "Welcome:" and "Wrap Up:" bookends of path 3546, which are not courses and
 *  are no longer in the data (see PathCourse). Rather than leave that arm dead,
 *  the grid now narrows to the badged units and data.test.ts asserts that a
 *  `coursesLayout: "badges"` path never has a badge-less course, so the two can
 *  never silently disagree. Badge-less units are still rendered — by
 *  CourseUnitsList, the layout Stanford's five modules use.
 *
 *  THE GLOW marks the lab-based skill badges — the two Credly-hosted ones — and
 *  is driven off `badge.provider`, never off a filename. It reuses the ledger's
 ...
 *  course badge page IS public, login-free and verifiable, so the pill names the
 *  platform the click verifies on — "Verify in Credly" / "Verify in Google
 *  Skills". The word is never applied to a path, only to a badge. */
export const CourseBadgesGrid = ({ path }: { path: LearningPathData }) => {
  // reducedBadgeVariants, never `undefined` — see the comment on that const.
  const badgeVariants = useReducedMotion() ? reducedBadgeVariants : panelBadgeVariants;
  // Narrow to the badged units, which on a "badges" path is every unit
  // (data.test.ts). The flatMap is what carries `badge` from optional to
  // present — no `course.badge!` anywhere in the closure below.
  const units = path.courses.flatMap((course) =>
    course.badge ? [{ ...course, badge: course.badge }] : [],
  );
            ...
            >
              ...
              <span className={COURSE_TILE_TEXT}>
                <span className="min-w-0 break-words t-small font-semibold text-ink/88">
                  {course.title}
                  {badge.linkTitle ? (
                    // The platform names this badge something else. Print
                    // BOTH, verbatim and IN FULL — never truncated, never
                    // clamped — so the destination corroborates the label it
                    // was clicked from. No t-* class here: it inherits the
                    // parent's t-small, the one token on this element.
                    <span className="text-ink/75 dark:text-ink/50">
                      {` (${badge.provider}: ${badge.linkTitle})`}
                    </span>
                  ) : null}
                </span>
                <span
                  className={cn(
                    ...
                    // longer provider-aware label WRAP inside the pill rather
                    // than overflow a narrow tile — the label is never
                    // truncated and never abbreviated. It sets on one line in
                    // the sideways unit at 375px and on two in a gallery column
              ...
              {/* `kind` changes ONLY this spoken suffix — a lab-based skill
                  ...
                  changes the suffix AND, on the one tile carrying a linkTitle,
                  visible copy above. The two fields are independent: read
                  both, derive neither. */}
