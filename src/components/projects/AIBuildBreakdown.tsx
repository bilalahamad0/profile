...
export function AIBuildBreakdown({
  ...
  return (
    <details className="group/ai mt-4 rounded-2xl border border-white/5 bg-white/[0.02] overflow-hidden">
      <summary className="flex items-center gap-2 px-4 py-3 cursor-pointer list-none select-none hover:bg-white/[0.03] transition-colors [&::-webkit-details-marker]:hidden">
        <Sparkles className={`w-3.5 h-3.5 shrink-0 ${colors.text}`} aria-hidden="true" />
        <span className="t-label font-black uppercase tracking-widest text-zinc-400">
          AI Build Breakdown
        </span>
        <ChevronDown
          className="w-4 h-4 ml-auto shrink-0 text-zinc-400 transition-transform group-open/ai:rotate-180"
          aria-hidden="true"
        />
      </summary>
      ...
      <div className="px-4 pb-4 pt-1 space-y-4">
        ...
        <dl className="grid grid-cols-2 sm:grid-cols-3 gap-2">
          {figures.map(({ label, value }) => (
            <div key={label} className={`p-2.5 rounded-xl ${colors.bg} border ${colors.border}`}>
              <dt className="t-label font-bold uppercase tracking-widest text-zinc-400">{label}</dt>
              <dd className="t-small font-semibold text-zinc-300 mt-0.5 leading-tight">{value}</dd>
            </div>
          ))}
        </dl>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          <div className="p-3 rounded-xl bg-red-500/5 border border-red-500/15">
            <span className="t-label font-black uppercase tracking-widest text-red-400 block mb-1.5">
              Before AI
            </span>
            <p className="t-caption text-zinc-400 leading-relaxed">{metrics.beforeAI}</p>
          </div>
          <div className={`p-3 rounded-xl ${colors.bg} border ${colors.border}`}>
            <span className={`t-label font-black uppercase tracking-widest ${colors.text} block mb-1.5`}>
              After AI
            </span>
            <p className="t-caption text-zinc-300 leading-relaxed">{metrics.afterAI}</p>
          </div>
        </div>
        ...
        {metrics.agents.length > 0 && (
          <div>
            <span className="block t-label font-black uppercase tracking-widest text-zinc-400 mb-2">
              AI Agents
            </span>
              ...
              {metrics.agents.map((agent) => (
                <li key={agent.name} className={`p-3 rounded-xl ${colors.bg} border ${colors.border}`}>
                  <div className="flex items-center gap-2 mb-1.5">
                    ...
                    <span className="t-small font-bold text-white">{agent.name}</span>
                    <span className="t-label text-zinc-400 ml-auto">{agent.period}</span>
                  </div>
                  <p className="t-label text-zinc-400 mb-1.5">{agent.provider}</p>
                  <div className="flex flex-wrap gap-1.5 mb-1.5">
                    {agent.models.map((m) => (
                      <span key={m} className={`px-2 py-0.5 rounded-md t-label font-bold bg-white/5 ${colors.text}`}>
                        {m}
                      </span>
                    ))}
                  </div>
                  <div className="flex items-baseline justify-between gap-3 t-label text-zinc-400">
        ...
        <p className="pt-3 border-t border-white/5 flex flex-wrap items-center justify-between gap-2 t-label text-zinc-400">
          <span>
            {projectName} metrics from <code className="text-zinc-400">ai-metrics.json</code>
          </span>
