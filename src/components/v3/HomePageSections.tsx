...
function SectionLabel({ color, children }: { color: string; children: React.ReactNode }) {
  ...
  return (
    <div className={`flex items-center gap-2 ${text[color] ?? "text-blue-700 dark:text-blue-500"} font-mono text-xs uppercase tracking-widest mb-3`}>
        ...
        >
          ...
          <motion.div variants={fadeUp} transition={{ duration: 0.5 }} className="flex justify-center">
            <Link
              ...
              className="group flex items-center gap-3 px-8 py-3.5 rounded-full bg-violet-500/10 border border-violet-500/20 text-ink font-bold hover:bg-violet-500/20 hover:border-violet-500/40 transition-all"
            >
              <Sparkles className="w-4 h-4 text-violet-700 dark:text-violet-400" />
              Explore AI Lab
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          <motion.div variants={fadeUp} transition={{ duration: 0.5 }}>
            ...
            <h2 id="blog-preview-heading" className="t-h2 text-ink mb-4">
              Lab{" "}
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-cyan-700 dark:from-blue-400 dark:to-cyan-400">Notes</span>
            </h2>
            <p className="text-ink-muted text-lg max-w-2xl mb-8 md:mb-12 lg:mb-14 leading-relaxed">
            ...
            {featured && (
              <motion.div variants={fadeUp} transition={{ duration: 0.5 }} className="lg:col-span-3">
                <Link
                  ...
                  className="group relative block rounded-2xl border border-violet-500/25 dark:border-violet-500/15 bg-gradient-to-br from-violet-100/60 to-indigo-100/40 dark:from-violet-950/30 dark:to-indigo-950/20 p-8 hover:border-violet-500/30 transition-all duration-500 h-full overflow-hidden"
                >
                  <div className="absolute top-0 right-0 w-48 h-48 bg-violet-500/10 rounded-full blur-[80px] pointer-events-none" />
                  <div className="relative z-10">
                    <div className="flex items-center gap-3 mb-5">
                      <span className="px-3 py-1 rounded-full bg-violet-500/15 border border-violet-500/25 t-label font-bold uppercase tracking-widest text-violet-700 dark:text-violet-300">
                        {featured.category}
                      </span>
                      <span className="text-xs text-ink-muted">{featured.readingTime} min read</span>
                    </div>
                    <h3 className="t-h3 text-ink mb-3 group-hover:text-violet-700 dark:group-hover:text-violet-200 transition-colors">
                      {featured.title}
                    </h3>
                    <p className="text-sm text-ink-muted leading-relaxed mb-6 line-clamp-3">{featured.description}</p>
                    <span className="flex items-center gap-2 text-sm font-semibold text-violet-700 dark:text-violet-400 group-hover:gap-3 transition-all">
            ...
            <motion.div variants={fadeUp} transition={{ duration: 0.5 }} className="lg:col-span-2 flex flex-col gap-4">
              {hasRecentPosts && recent.map((post) => (
                <Link
                  ...
                  className="group flex-1 rounded-2xl border border-line/10 dark:border-line/[0.06] bg-surface-card dark:bg-ink/[0.02] p-6 hover:bg-ink/[0.04] hover:border-line/[0.12] transition-all duration-500"
                >
                  <div className="flex items-center gap-2 mb-3">
                    <FileText className="w-3.5 h-3.5 text-blue-700 dark:text-blue-400" />
                    <span className="t-label font-bold uppercase tracking-widest text-blue-700 dark:text-blue-400">{post.category}</span>
                    <span className="t-label text-ink-muted ml-auto">{post.readingTime} min</span>
                  </div>
                  <h3 className="text-sm font-bold text-ink mb-2 group-hover:text-blue-800 dark:group-hover:text-blue-300 transition-colors line-clamp-2">{post.title}</h3>
                  <p className="text-xs text-ink-muted line-clamp-2">{post.description}</p>
                </Link>
              ))}
              {!hasRecentPosts && latestLinkedIn.map((post) => (
                ...
                >
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="t-label font-bold uppercase tracking-widest text-blue-700 dark:text-blue-400">LinkedIn</span>
                  </div>
                  <h3 className="text-sm font-bold text-ink mb-2 group-hover:text-blue-800 dark:group-hover:text-blue-300 transition-colors line-clamp-2">{post.title}</h3>
          ...
          <motion.div variants={fadeUp} transition={{ duration: 0.5 }} className="mt-6 md:mt-8 lg:mt-10 flex justify-center">
            <Link
              href="/blog"
              className="group flex items-center gap-3 px-8 py-3.5 rounded-full bg-ink/[0.05] border border-line/10 text-ink font-bold hover:bg-ink/[0.1] hover:border-line/20 transition-all"
            >
    ...
    <section className="px-6 lg:px-24 py-12 md:py-20 lg:py-24 relative overflow-hidden" aria-label="Contact and availability">
      ...
      <div className="max-w-4xl mx-auto relative z-10">
        ...
        >
          ...
          <motion.h2 variants={fadeUp} transition={{ duration: 0.5 }} className="t-h2 text-ink mb-6">
            Let&apos;s Build Something{" "}
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 via-violet-600 to-cyan-700 dark:from-blue-400 dark:via-violet-400 dark:to-cyan-400">Together</span>
          </motion.h2>

          <motion.p variants={fadeUp} transition={{ duration: 0.5 }} className="t-lead text-ink-muted max-w-2xl mx-auto mb-8">
          ...
          <motion.div variants={fadeUp} transition={{ duration: 0.5 }} className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-8 md:mb-10 lg:mb-12">
            <Link
              href="/contact"
              className="group flex items-center gap-3 px-8 py-4 rounded-full bg-ink text-surface font-bold hover:bg-ink/85 dark:hover:bg-zinc-200 transition-all shadow-[0_0_30px_rgba(255,255,255,0.1)] hover:shadow-[0_0_40px_rgba(255,255,255,0.2)] hover:scale-105 active:scale-95"
            >
              Get in Touch
            ...
            <a
              href={BOOKING_ANCHOR}
              className="group flex items-center gap-3 px-8 py-4 rounded-full bg-ink/[0.05] border border-line/10 text-ink font-bold hover:bg-ink/[0.1] hover:border-line/20 transition-all"
            >
              <CalendarClock className="w-4 h-4 text-blue-700 dark:text-blue-400" />
              Book a Call
              <ArrowRight className="w-4 h-4 text-ink-muted group-hover:translate-x-1 transition-transform" />
            </a>
          </motion.div>
