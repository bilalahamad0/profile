...
date: "2026-04-30"
description: "Building a highly reliable, zero-latency IoT system on a Raspberry Pi: mDNS discovery, background compute windows, and self-healing multi-tier fallbacks."
...
Through engineering the Adhan Caster, I tackled several "gotchas" that are prevalent in the IoT space. Here are the core industry-standard practices and performance optimizations required to build truly efficient, production-grade IoT functionality.
...
By implementing aggressive state caching, asynchronous pre-flight windows, hardware awareness, and independent auditing, the Adhan Caster holds a 99% cast success rate at roughly 2.1 s average latency. Those are the numbers the [live operations dashboard](https://bilalahamad0.github.io/adhan-api/dashboard.html) reports, not a target — and the missing 1% is a real incident, written up in [the clock-jump case study](/blog/clock-jump-case-study).
