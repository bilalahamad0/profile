...
This repository contains a modern personal website for Bilal Ahamad's resume. It uses **Tailwind CSS** for styling and includes a dark/light theme toggle with state saved to `localStorage`. When dark mode is activated the `body` element receives the classes `page-overview theme-dark` to enable a rich night theme. The layout now adopts a glossy glassmorphism aesthetic for a sleek appearance.
